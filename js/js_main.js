...
function prixVoyage() { //calcul le prix du voyage et le met à jour en direct
    ...
    nbAdultes = nbAdultes.value;    //nombre d'adultes
...
function returnStorage() {  //retourne le prix et le numéro de résa des commandes effectuées à partir du storage sous forme de listes
    ...
    return [listePrix, listeLieu, listeNumResa];
}

function affichagePanier() {    //ajoute à la page du panier les destinations présentent dans le localStorage
    let listeTemp = returnStorage();
    let listePrix = listeTemp[0];
    let listeLieu = listeTemp[1];
    let listeNumResa = listeTemp[2];
    let nouveauHTML = "";

    for ( let indiceResa = 0; indiceResa < listeLieu.length; indiceResa++) {
        //let lieu1 = listeLieu[indiceResa];
        let lieu1 = "";
        for (let lettre of listeLieu[indiceResa]) {
            if (lettre === " ") {
                lieu1 = lieu1 + " ";
            } else {
                lieu1 = lieu1 + lettre;
        ...
        let strOnclick = "supprPanier("+lieu1+");";

        let lieuDebug = "'" + listeLieu[indiceResa] + "'";
        let bouton = "<button" + " " + "id=" + lieuDebug + " " + "onclick="+strOnclick+">Supprimer</button>";
        let division = "<div><p>" + "Réservation n°" + listeNumResa[indiceResa] + " pour " + listeLieu[indiceResa] + " : " + listePrix[indiceResa] + "€" + "</p>" + bouton + "</div>";
        nouveauHTML = nouveauHTML + division;
    }
    document.getElementById("section_panier").innerHTML = nouveauHTML;
    //localStorage.clear();
}

function supprPanier(lieu) {
    //console.log(lieu);
    //let lieuTEST = "";
    //for (let lettre of lieu) {
    //    if (lettre === "_") {
    //        lieuTEST = lieuTEST + " ";
    //    } else {
